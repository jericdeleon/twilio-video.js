...
`;
    ...
    const filteredLocalSdp = filterLocalCodecs(localSdp, remoteSdp);
...
describe('revertSimulcast', () => {
  ...
  ], ([isVP8PreferredPayloadType, revertForAll, ssrcs]) => {
    let sdp;
    let simSdp;
    let remoteSdp;
    let revertedSdp;
...
describe('addOrRewriteNewTrackIds', () => {
  [true, false].forEach(withAppData => {
    context(`when the SDP ${withAppData ? 'has' : 'does not have'} MediaStreamTrack IDs in a=msid: lines`, () => {
      it('should rewrite Track IDs with the IDs of MediaStreamTracks associated with unassigned RTCRtpTransceivers', () => {
        ...
        const newSdp = addOrRewriteNewTrackIds(sdp, activeMidsToTrackIds, newTrackIdsByKind);
...
describe('addOrRewriteTrackIds', () => {
  [true, false].forEach(withAppData => {
    context(`when the SDP ${withAppData ? 'has' : 'does not have'} MediaStreamTrack IDs in a=msid: lines`, () => {
      it('should rewrite Track IDs with the IDs of MediaStreamTracks associated with RTCRtpTransceivers', () => {
        const sdp = makeSdpWithTracks({ audio: ['foo'], video: ['bar'] }, null, null, withAppData);
